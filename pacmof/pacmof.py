...
#%%
		# * Small Z
	def find_neighbors_smallZ(i, struct_dict):
		import numpy as np
...
#%%
	...
	# %%
	# * Large Z
	def find_neighbors_largeZ(i, struct_dict):
		import numpy as np
	...
	def find_neighbors_oxynitro(i, struct_dict):
		import numpy as np
		from ase.data import covalent_radii
		from pymatgen.io import ase
		from pymatgen import Structure
...
#%%
	...
	struct_dict = mof_pm.as_dict()
	global_struct = Variable('atoms_ase')
	global_struct.set(struct_dict)

	# Divide the calculation into two groups and treate the calculations separately.
	import dask.bag as db
	indices1_bag = db.from_sequence(np.array(atom_ids[np.where(np.array(flags) == 1)]), npartitions=100).map_partitions(lambda chk: [find_neighbors_smallZ(x, struct_dict = global_struct) for x in chk])
	indices2_bag = db.from_sequence(np.array(atom_ids[np.where(np.array(flags) == 2)]), npartitions=100).map_partitions(lambda chk: [find_neighbors_oxynitro(x, struct_dict = global_struct) for x in chk])
	indices3_bag = db.from_sequence(np.array(atom_ids[np.where(np.array(flags) == 3)]), npartitions=100).map_partitions(lambda chk: [find_neighbors_largeZ(x,  struct_dict = global_struct) for x in chk])


	index_list =np.array(np.where(np.array(flags) == 1)[0].tolist() + np.where(np.array(flags) == 2)[0].tolist()+np.where(np.array(flags) == 3)[0].tolist())
	...
	print("Nearest 2 neighbors are considered since no chemically bonded neighbors are found for {} atoms in this case.".format(len(no_neighbors)))
...
# %%
def get_charges_multiple_parallel(list_of_cifs,  create_cif=False, path_to_output_dir='.', add_string='_charged', use_default_model=True, path_to_pickle_obj='dummy_string', client_name='dummy'):
